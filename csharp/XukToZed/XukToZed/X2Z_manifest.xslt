...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xuk="http://www.daisy.org/urakawa/xuk/0.5" xmlns:obi="http://www.daisy.org/urakawa/obi/0.5" >
  ...
  <xsl:template match="*[@src]" mode="MANIFEST">
    <item xmlns="http://openebook.org/namespaces/oeb-package/1.0/">
      <xsl:attribute name="id">
        <xsl:choose>
          <xsl:when test="contains(@src,'.wav') or contains(@src,'.mp3')">aud_<xsl:value-of select="@src"/></xsl:when>
  ...
  <xsl:template match="xuk:CoreNode[xuk:mProperties/obi:info[@type='Section'] | preceding-sibling::xuk:mProperties/obi:info[@type='Section'][1]]" mode="MANIFEST">
    <item xmlns="http://openebook.org/namespaces/oeb-package/1.0/">
      <xsl:attribute name="id">SMIL_<xsl:value-of select="generate-id((ancestor-or-self::xuk:CoreNode[xuk:mProperties/obi:info[@type='Section'] or preceding-sibling::xuk:CoreNode[xuk:mProperties/obi:info[@type='Section']]][1]))"/></xsl:attribute>
