...
namespace urakawa.unitTests.mediaDataTests
{
	...
	public class EmptyMediaDataPresentationTests
	{
		...
		public void SplitAudio()
		{
			...
			double initMSecs = mams[0].Duration.getTimeDeltaAsMillisecondFloat();
			double msecs, diff;
			for (int i = 0; i < 6; i++)
			{
				msecs = mams[i].Duration.getTimeDeltaAsMillisecondFloat();
				mams.Add(mams[i].Split(new Time(msecs / 2)));
				diff = Math.Abs((msecs / 2) - mams[i].Duration.getTimeDeltaAsMillisecondFloat());
				Assert.Less(
					diff, 0.1,
					"The difference the split ManagedAudioMedia actual and expec duration is more than 0.1ms");
				diff = Math.Abs((msecs / 2) - mams[i + 1].Duration.getTimeDeltaAsMillisecondFloat());
				Assert.Less(
					diff, 0.1,
					"The difference the split ManagedAudioMedia actual and expec duration is more than 0.1ms");
			}
			...
			{
				double s = m.Duration.getTimeDeltaAsMillisecondFloat();
				msecs += s;
			}
