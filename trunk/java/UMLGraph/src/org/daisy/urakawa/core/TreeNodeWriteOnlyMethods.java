...
/**
 * Write-only TreeNode methods. It is convenience interface for the design only,
 * in order to organize the data model in smaller modules.
 * 
...
public interface TreeNodeWriteOnlyMethods {
	/**
	 * This method should only be used internally (within the descendant class
	 * hierarchy), not by third-party application developers. In fact it should
	 * be protected or private (but we're in an interface, so...).
	 * 
	 * @param node
	 *            the new parent for this node. can be NULL.
	 */
	public void setParent(TreeNode node);

	/**
	 * Inserts the given TreeNode as a child of this node, at the given index.
	 * Does NOT replace the existing child, but increments (+1) the indexes of
	 * all children which index is >= insertIndex. If insertIndex ==
	 * children.size (no following siblings), then the given node is appended at
	 * the end of the existing children list.
	 * 
	 * @param node
	 *            cannot be null
	 * @param insertIndex
	...
	/**
	 * Inserts a new child TreeNode before (sibbling) a given reference child
	 * TreeNode.
	 * 
	 * @param node
	 *            cannot be null
	...
	/**
	 * Inserts a new child TreeNode after (sibbling) a given reference child
	 * TreeNode.
	 * 
	 * @param node
	 *            cannot be null
	 ...
	 * @throws MethodParameterIsNullException
	 *             NULL method parameters are forbidden
	 * @throws throws TreeNodeDoesNotExistException
	 * @throws TreeNodeIsInDifferentPresentationException
	 * @throws TreeNodeHasParentException
	 * @throws TreeNodeIsAncestorException
	 * @throws TreeNodeIsSelfException
	...
	/**
	 * Removes the child TreeNode at a given index (as a whole sub-tree, "deep"
	 * tree operation).
	 * 
	 * @param index
	 *            must be in bounds [0..children.size-1].
	...
	/**
	 * Removes a given child TreeNode (as a whole sub-tree, "deep" tree
	 ...
	 *         its properties (see the "copyProperties" mathod parameter), which
	 *         has all children of [this] node starting at [index] up to
	 *         [getChildCount()-1]. [This] node looses these children, but
	 *         retains the previous sibling ones (from [0] to [index-1]).
	 * @tagvalue Exceptions "MethodParameterIsOutOfBounds"
	 * @throws MethodParameterIsOutOfBoundsException
	 */
