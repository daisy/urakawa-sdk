...
public interface ITreeNodeReadOnlyMethods {
	/**
	 * Creates a new ITreeNode with identical content (recursively) as this node,
	 * but compatible with the given IPresentation (factories, managers,
	 ...
	 * contents) is not compatible with the given destination IPresentation (i.e.
	 * an attempt to create a copy using a factory with a given QName, fails),
	 * then the FactoryCannotCreateTypeException is raised.
	 * 
	 * @param destPres
	 *            the destination IPresentation to which this node (and all its
	 *            content, recursively) should be exported.
	 * @return a new ITreeNode with identical content (recursively) as this node,
	 *         but compatible with the given IPresentation (factories, managers,
