...
namespace javazoom.jl.decoder
{
	...
	public sealed class Bitstream : BitstreamErrors
	{
		...
		public Header readFrame()
		{
			...
			try
			{
                ////System.Diagnostics.Trace.WriteLine("BitStream - reading next frame");
				result = readNextFrame();
			}
			catch (BitstreamException ex)
			{
                ////System.Diagnostics.Trace.WriteLine("BitStream - error raised");
				if (ex.ErrorCode != javazoom.jl.decoder.BitstreamErrors_Fields.STREAM_EOF)
				{
					// wrap original exception so stack trace is maintained.
                    ////System.Diagnostics.Trace.WriteLine("BitStream - throwing error");
					throw newBitstreamException(ex.ErrorCode, ex);
				}
			}
            ////System.Diagnostics.Trace.WriteLine("BitStream - readFrame - end");
			return result;
		}
		...
		private Header readNextFrame()
		{
			if (framesize == - 1)
			{
                ////System.Diagnostics.Trace.WriteLine("BitStream - readNextFrame - reading next frame");
				nextFrame();
			}
		...
		private void  nextFrame()
		{
            ...
            ////System.Diagnostics.Trace.WriteLine("BitStream - nextFrame - read_header");
		...
		public int syncHeader(sbyte syncmode)
		{
            ...
            ////System.Diagnostics.Trace.WriteLine("BitStream - syncHeader - readbytes");
            ...
            ////System.Diagnostics.Trace.WriteLine("BitStream - syncHeader - headerstring populated");
			...
			do 
			{
                ////System.Diagnostics.Trace.WriteLine("BitStream - syncHeader - looping");
				headerstring <<= 8;
                ...
                ////System.Diagnostics.Trace.WriteLine("BitStream - syncHeader - isSyncMark");
				sync = isSyncMark(headerstring, syncmode, syncword);
...
#endif
                ////System.Diagnostics.Trace.WriteLine("BitStream - syncHeader - attempt:"+attempts.ToString()+ " are we synced yet? " + sync.ToString());
                //attempts++;
			}
            while (!sync);//&& attempts<1000
		...
		private int readBytes(sbyte[] b, int offs, int len)
		{
			...
			try
			{
                ////System.Diagnostics.Trace.WriteLine("BitStream - readBytes - enter loop");
				while (len > 0)
				{
                    ////System.Diagnostics.Trace.WriteLine("BitStream - readBytes - read from source object");
					int bytesread = source.Read(b, offs, len);
//					for (int i = 0; i < len; i++) b[i] = (sbyte)Temp[i];
                ...
                ////System.Diagnostics.Trace.WriteLine("BitStream - readBytes - end loop");

			}
			catch (System.IO.IOException ex)
			{
                ////System.Diagnostics.Trace.WriteLine("BitStream - readBytes - error");
				throw newBitstreamException(javazoom.jl.decoder.BitstreamErrors_Fields.STREAM_ERROR, ex);
			}
            ////System.Diagnostics.Trace.WriteLine("BitStream - readBytes - exit");
