...
namespace NLayer.Decoder
{
    ...
    public sealed class Bitstream
    {
         ...
         */
        private readonly uint[] framebuffer = new uint[BUFFER_INT_SIZE];
        ...
        private readonly uint[] bitmask = {0,	// dummy
     ...
     0x0001FFFF };
        ...
        {
            if (inStream == null) throw new ArgumentNullException("in");
            inStream = new BufferedStream(inStream);
            loadID3v2(inStream);
            firstframe = true;
            //source = new PushbackInputStream(in, 1024);
            source = new PushbackInputStream(inStream, BUFFER_INT_SIZE * 4);
        ...
        private void loadID3v2(Stream inStream)
        {
            ...
            try
            {
                if (size > 0)
                {
                    ...
                    inStream.Read(rawid3v2, 0, rawid3v2.Length);
                }
            }
            catch (IOException e)
            { }
        ...
        public Stream getRawID3v2()
        {
            ...
            {
                MemoryStream bain = new MemoryStream(rawid3v2);
                return bain;
            }
        }
        ...
        public void close()
        {
            ...
            {
                throw new BitstreamException(Errors.STREAM_ERROR, ex);
            }
        }
        ...
        public Header readFrame()
        {
            ...
            {
                if ((ex.Error == Errors.INVALIDFRAME))
                {
                    ...
                    {
                        if ((ex.Error != Errors.STREAM_EOF))
                        {
                            // wrap original exception so stack trace is maintained.
                            throw new BitstreamException(ex.Error, e);
                        }
                    }
                }
                else if ((ex.Error != Errors.STREAM_EOF))
                {
                    // wrap original exception so stack trace is maintained.
        ...
        public bool isSyncCurrentPosition(int syncmode)
        {
            ...
            uint headerstring = (((uint)syncbuf[0] << 24) & 0xFF000000) | (((uint)syncbuf[1] << 16) & 0x00FF0000) | (((uint)syncbuf[2] << 8) & 0x0000FF00) | (((uint)syncbuf[3] << 0) & 0x000000FF);
        ...
        public int syncHeader(byte syncmode)
        {
            ...
            if (bytesRead != 3) throw new BitstreamException(Errors.STREAM_EOF);
            ...
            do
            {
                ...
                if (readBytes(syncbuf, 3, 1) != 1)
                    throw new BitstreamException(Errors.STREAM_EOF);

                headerstring |= (syncbuf[3] & 0x000000FF);

                sync = isSyncMark((uint)headerstring, syncmode, syncword);
            }
            while (!sync);
        ...
        public bool isSyncMark(uint headerstring, int syncmode, uint word)
        {
            bool sync = false;
         ...
         */
        public int read_frame_data(int bytesize)
        {
            int numread = 0;
        ...
        public void parse_frame()
        {
            ...
            for (int k = 0; k < bytesize; k = k + 4)
            {
                ...
                framebuffer[b++] = (((uint)b0 << 24) & 0xFF000000) | (((uint)b1 << 16) & 0x00FF0000) | (((uint)b2 << 8) & 0x0000FF00) | ((uint)b3 & 0x000000FF);
            }
            wordpointer = 0;
            bitindex = 0;
        }
        ...
        public int get_bits(int number_of_bits)
        {
            ...
            uint Right = (framebuffer[wordpointer] & 0x0000FFFF);
            wordpointer++;
            uint Left = (framebuffer[wordpointer] & 0xFFFF0000);
         ...
         */
        public void set_syncword(uint syncword0)
        {
