...
namespace NLayer.Decoder
{
    ...
    public sealed class Header
    {
            ...
            try
            {
                ...
                if (xing.Equals(Encoding.ASCII.GetString(tmp)))
                {
                    ...
                    if ((flags[3] & (byte)(1 << 0)) != 0)
                    {
                        ...
                        h_vbr_frames = (int)(((uint)tmp[0] << 24) & 0xFF000000 | ((uint)tmp[1] << 16) & 0x00FF0000 | ((uint)tmp[2] << 8) & 0x0000FF00 | (uint)tmp[3] & 0x000000FF);
                        length += 4;
                    }
                    ...
                    {
                        Array.Copy(firstframe, offset + length, tmp, 0, tmp.Length);
                        h_vbr_bytes = (int)(((uint)tmp[0] << 24) & 0xFF000000 | ((uint)tmp[1] << 16) & 0x00FF0000 | ((uint)tmp[2] << 8) & 0x0000FF00 | (uint)tmp[3] & 0x000000FF);
                        length += 4;
                    }
                    ...
                    {
                        Array.Copy(firstframe, offset + length, tmp, 0, tmp.Length);
                        h_vbr_scale = (int)(((uint)tmp[0] << 24) & 0xFF000000 | ((uint)tmp[1] << 16) & 0x00FF0000 | ((uint)tmp[2] << 8) & 0x0000FF00 | (uint)tmp[3] & 0x000000FF);
                        length += 4;
                    }
         ...
         * Returns Copyright.
         */
        public bool copyright() { return h_copyright; }

        /**
         * Returns Original.
         */
        public bool original() { return h_original; }

        /**
         * Return VBR.
         * @return true if VBR header is found
         */
        public bool vbr() { return h_vbr; }
         ...
         */
        public bool checksum_ok() { return (checksum == crc.checksum()); }
