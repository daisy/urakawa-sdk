...
namespace urakawa.daisy.export.visitor
{
    public abstract class AbstractPublishFlattenedManagedAudioVisitor : AbstractBasePublishAudioVisitor
    {
        ...
        private void checkTransientWavFileAndClose(TreeNode node)
        {
            ...
            if (m_ExternalAudioMediaList.Count > 0)
            {
                    ...
                    nChannels != node.Presentation.MediaDataManager.DefaultPCMFormat.Data.NumberOfChannels)
                {
                    ...
                    {
                        EncodeTransientFile(node);
                    }
                    else
                    {
                        EncodeTransientFileResample(node);
                    }
                }
        ...
        private void EncodeTransientFile(TreeNode node)
        {   
            if (EncodingFileFormat == AudioFileFormats.MP3)
            {
                EncodeTransientFileToMp3(node);
            }
            ...
            {
                EncodeTransientFileToMp4OrAMR(node);
            }
        }


        private void EncodeTransientFileToMp3(TreeNode node)
        {
            ExternalAudioMedia extMedia = m_ExternalAudioMediaList[0];
            ...
            try
            {
                result = formatConverter.CompressWavToMp3(sourceFilePath, destinationFilePath, pcmFormat,Convert.ToUInt16 (BitRate_Encoding), m_AdditionalMp3ParamChannels,m_AdditionalMp3ParamReSample, m_AdditionalMp3ParamReplayGain);
            }
            finally
            {
                RemoveSubCancellable(formatConverter);
