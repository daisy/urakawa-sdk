...
import org.daisy.urakawa.events.media.data.audio.PCMFormatChangedEvent;
        ...
        implements IAudioMediaData
{
            ...
            throws MethodParameterIsNullException
    {
        ...
        if (PCMFormatChangedEvent.class.isAssignableFrom(klass))
        {
            ...
            if (AudioDataInsertedEvent.class.isAssignableFrom(klass))
            {
                mAudioDataInsertedEventNotifier.registerListener(listener,
                        klass);
            }
            else
                if (AudioDataRemovedEvent.class.isAssignableFrom(klass))
                {
                    mAudioDataRemovedEventNotifier.registerListener(listener,
                            klass);
                }
                else
                {
            ...
            throws MethodParameterIsNullException
    {
        ...
        if (PCMFormatChangedEvent.class.isAssignableFrom(klass))
        {
            ...
            if (AudioDataInsertedEvent.class.isAssignableFrom(klass))
            {
                mAudioDataInsertedEventNotifier.unregisterListener(listener,
                        klass);
            }
            else
                if (AudioDataRemovedEvent.class.isAssignableFrom(klass))
                {
                    mAudioDataRemovedEventNotifier.unregisterListener(listener,
                            klass);
                }
                else
                {
    ...
    public IPCMFormatInfo getPCMFormat()
    {
        if (mPCMFormat == null)
        {
            try
            {
                mPCMFormat = new PCMFormatInfo(getPresentation()
            ...
            TimeOffsetIsOutOfBoundsException, FactoryCannotCreateTypeException
    {
        ...
        try
        {
            md = getPresentation().getMediaDataFactory().create(
